...
class NodeArticle {
    ...
    fetch() {
        return this.#getDom().then(dom => {
            this.title = this.#getTitle(dom);
            this.infobox = this.#getInfobox(dom);
            
            const content = this.#getContent(dom);
     ...
     */
    #getTitle(dom) {
        return dom.window.document.getElementById('firstHeading').textContent;
    }
